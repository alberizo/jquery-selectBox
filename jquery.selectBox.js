    ...
    $.extend($.fn, {
        selectBox: function(method, data) {
            ...
            var init = function(select, data) {
                ...
                control.data('selectBox-settings', settings)

                if(typeof select.data('autoWidth') == 'undefined'){
                    ...
                    select.children('OPTION, OPTGROUP').each(function() {
                        ...
                        if ($(this).is('OPTION')) {
                            ...
                            if ($(this).length > 0) {
                                ...
                                options.find('ul').append('<li>\u00A0</li>');
                            }
                        } else {
                            // If the element is an option group, add the group and call this function on it.
                            var optgroup = $('<li class="selectBox-optgroup" />');
                            optgroup.text($(this).attr('label'));
                            options.find('ul').append(optgroup);
            ...
            var showMenu = function(select) {
                ...
                var select_width = select.data('selectBox-control').innerWidth();
                var autoWidth = select_width;

                if(select.data('autoWidth')){
                    if(options.width() - 10 > select_width){
                        autoWidth = 'auto';
                        options.css('minWidth', select_width)
                        ...
                        input_filter.not('.search_plugin')
                            .search(li_list, function(on){
                                ...
                                on.results(function(results){
                                    ...
                                    var li_list_results = li_list.filter(results)
                                    // If there are OPTGROUPs in select then filter is different, show headers
                                    if(li_list.filter('.selectBox-optgroup').length > 0){
                                        li_list_results.show();

                                        li_list_results.each(function(){
                                            if($(this).hasClass('selectBox-optgroup')){
                                                var next = $(this).nextUntil('.selectBox-optgroup');
                                                if(next.length > 0){
                                                    next.show();
                                                }
                                            }else{
                                                $(this).prevUntil('ul').filter('.selectBox-optgroup:first').show();
                                            }
                                        })
